...
async function generateConsentPDF(params: {
  ...
  page.drawRectangle({ x: 0, y: pageHeight - 80, width: pageWidth, height: 80, color: rgb(0.04, 0.04, 0.08) });
  ...
  page.drawRectangle({ x: pageWidth - marginRight - 120, y: pageHeight - 68, width: 120, height: 28, color: rgb(0.2, 0.8, 0.4) });
  ...
  const agreementSections = [
    { title: '1. CONSENT TO RECORDING', body: 'I voluntarily consent to being recorded, photographed, and/or appearing in digital content ("Content") created by the Creator.' },
    { title: '2. RIGHTS & LICENSE', body: 'I grant an irrevocable, perpetual, worldwide, royalty-free license to: use, reproduce, distribute, display, and monetize the Content; modify, edit, or adapt the Content; and publish the Content on Yonkamania and any other platforms.' },
    { title: '3. AGE & ID VERIFICATION', body: 'I confirm that I am at least 18 years of age, I possess valid government-issued identification, and I am legally able to consent. I acknowledge that false statements may result in legal consequences.' },
    { title: '4. CONSENT TO DISTRIBUTION (INCLUDING PAID CONTENT)', body: 'I understand the Content may be sold or monetized, distributed publicly or privately, and accessed by paying users.' },
    { title: '5. NO OWNERSHIP CLAIM', body: 'I waive any rights to ownership of the Content and future compensation (unless separately agreed).' },
    { title: '6. RELEASE OF LIABILITY', body: 'I release and hold harmless the Creator, Yonkamania, and their affiliates and partners from any claims related to the use, distribution, or monetization of the Content.' },
    { title: '7. VOLUNTARY AGREEMENT', body: 'I confirm I am signing willingly and that no coercion or pressure was involved.' },
    { title: '8. DIGITAL SIGNATURE AGREEMENT', body: 'I agree that my electronic signature is legally binding and that this agreement is enforceable as a written contract.' },
  ];
  ...
  if (params.signatureImageBase64) {
    try {
      ...
      const imgBytes = Uint8Array.from(atob(base64Data), (char) => char.charCodeAt(0));
  ...
  for (let index = 0; index < pageCount; index += 1) {
    ...
    currentPage.drawLine({ start: { x: marginLeft, y: 45 }, end: { x: width - marginRight, y: 45 }, thickness: 0.5, color: rgb(0.8, 0.8, 0.8) });
    currentPage.drawText('Yonkamania Creator Studio - Legally binding digital consent form', { x: marginLeft, y: 32, size: 7, font: helvetica, color: rgb(0.5, 0.5, 0.5) });
    currentPage.drawText(`Page ${index + 1} of ${pageCount}`, { x: width - marginRight - 50, y: 32, size: 7, font: helvetica, color: rgb(0.5, 0.5, 0.5) });
  }
...
consentRouter.get('/sign/:token', async (c) => {
  ...
  if (form.status === 'pending') {
    await supabaseAdmin.from('consent_forms').update({ status: 'viewed', viewed_at: new Date().toISOString() }).eq('token', token);
  }

  return c.json({ data: { ...form, creatorName, alreadySigned: false } });
});
...
consentRouter.post(
  ...
  async (c) => {
    ...
    const { data: form, error: formError } = await supabaseAdmin.from('consent_forms').select('*').eq('token', token).single();
    ...
    const signerIp = c.req.header('x-forwarded-for')?.split(',')[0]?.trim() ?? c.req.header('x-real-ip') ?? c.req.header('cf-connecting-ip') ?? 'unknown';
    const signerUserAgent = c.req.header('user-agent') ?? 'unknown';

    const { data: creator } = await supabaseAdmin.from('profiles').select('display_name, username').eq('id', form.creator_id).single();
    const creatorName = creator?.display_name ?? creator?.username ?? 'Unknown Creator';
    ...
    if (signature_image_base64) {
      try {
        ...
        const { error: uploadErr } = await supabaseAdmin.storage.from('consent-signatures').upload(fileName, imgBytes, { contentType: 'image/png', upsert: true });
        if (!uploadErr) {
    ...
    try {
      ...
      const { error: pdfErr } = await supabaseAdmin.storage.from('consent-pdfs').upload(pdfFileName, pdfBytes, { contentType: 'application/pdf', upsert: true });
